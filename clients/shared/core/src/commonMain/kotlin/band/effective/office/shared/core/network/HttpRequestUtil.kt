...
/**
 * Utility object for making HTTP requests and handling responses
 * Provides a unified way to make requests with automatic error handling
 */
object HttpRequestUtil {
     ...
     * 
     * @param client HttpClient instance (defaults to HttpClientProvider.create())
     * @param url URL to request
     * @param method HTTP method to use (GET, POST, PUT, DELETE)
     * @param block Request configuration block (headers, body, etc.)
     * @return Result<T> with either Success containing data or Error with code and message
     * 
        ...
        try {
            ...
            if (response.status.value in 200..299) {
                ...
                if (response.status == HttpStatusCode.NoContent || response.status == HttpStatusCode.Created) {
                    // If expecting SuccessResponse, return it directly
